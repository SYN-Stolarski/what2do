...
export function Participant({ session }: { session: Session }) {
  const { answers, step, screen, hasProgress, setAnswer, go, reset } = useAnswers(session.id)
  ...
  if (screen === 'summary') {
    return (
      <div className="app">
        ...
        <Summary questions={questions} answers={answers} onEdit={(i) => go('question', i)} onSubmit={submit} />
      </div>
    )
  }
